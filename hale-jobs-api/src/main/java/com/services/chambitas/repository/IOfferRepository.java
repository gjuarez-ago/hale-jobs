...
public interface IOfferRepository extends JpaRepository<Offer, Long>{
	...
	@Query(value = "SELECT o FROM Offer o JOIN o.user u JOIN o.category c JOIN o.subcategory s JOIN o.rangeAmount r JOIN o.levelStudy l JOIN o.typeOfJob t WHERE u.id = :userId AND c.id = 1 AND o.title LIKE %:title% AND s.valor LIKE %:subcategory% AND r.clave LIKE %:range_amount% AND o.status = :status AND o.urgency LIKE %:urgency% AND o.workPlace LIKE %:work_place% AND l.clave LIKE %:level_study% AND t.clave LIKE %:type_job% AND o.vencimiento >= NOW() AND o.regBorrado = 0")
	...
	@Query(value = "SELECT o FROM Offer o JOIN o.category c JOIN o.subcategory s JOIN o.rangeAmount r JOIN o.typeOfJob t JOIN o.state st WHERE c.id = :category AND o.title LIKE %:title%  AND s.valor LIKE %:subcategory% AND r.clave LIKE %:range_amount% AND o.status = 0 AND st.clave LIKE %:state% AND o.urgency LIKE %:urgency% AND t.clave LIKE %:type_job% AND o.vencimiento >= NOW() AND o.regBorrado = 0 ORDER BY o.regDateCreated DESC")
	...
	@Query(value = "SELECT o FROM Offer o JOIN o.company c WHERE o.status = 0 AND c.id = :company AND o.vencimiento >= NOW() AND o.regBorrado = 0 ORDER BY o.regDateCreated ASC")
	...
	@Query(value = "SELECT o FROM Offer o JOIN o.user u WHERE o.status = 0 AND u.id = :user AND o.vencimiento >= NOW() AND o.regBorrado = 0 ORDER BY o.regDateCreated ASC")
 	List<Offer> getOffersBySelect(@Param("user") Long user);
	
	// Consulta para que el administrador pueda ver las solicitudes.
	@Query(value = "SELECT o.* FROM offer AS o WHERE o.user_id = :title AND o.vencimiento >= NOW()  AND o.reg_borrado = 0",nativeQuery = true)
