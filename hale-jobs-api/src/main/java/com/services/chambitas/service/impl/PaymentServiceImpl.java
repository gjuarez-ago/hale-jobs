...
public class PaymentServiceImpl implements IPaymentService{
	...
	public Payment generatePayment(PaymentDTO request) {
		...
		User user = userRepository.findUserByConsecutive(request.getUserId());
		...
		entity.setRegCreatedBy(request.getRegCreatedBy());
	...
	@Override
	public Page<Payment> getPaymentByLoanIdW(int pageNo, int pageSize,String loanId) {
		Pageable pageable = PageRequest.of(pageNo, pageSize);   
		Page<Payment> response = repository.findPaymentsByLoanIdW(loanId, pageable);
		return response;	
	}
	...
	@Override
	public Payment deletePayment(String consecutive, String currentUser) throws GenericException {
		
		Payment entity = existPayment(consecutive);
		entity.setRegBorrado(1);
		entity.setRegDateUpdated(new Date());
		entity.setRegUpdateBy(currentUser);
		return entity;
	}
	...
	@Override
	public Page<Payment> getPaymentByLoanIdWA(int pageNo, int pageSize, String loanId) {
		Pageable pageable = PageRequest.of(pageNo, pageSize);   
		Page<Payment> response = repository.findPaymentsByLoanIdWA(loanId, pageable);
		return response;	
	}
