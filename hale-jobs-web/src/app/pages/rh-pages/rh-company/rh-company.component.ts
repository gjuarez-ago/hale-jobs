...
export class RhCompanyComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.title.setTitle('Hale | Mis empresas');
  ...
  private deleteUserById(element: any) {
    ...
    this.isLoadingGeneral = true;
    this.subscriptions.push(
      this.companyService.deleteCompanyById(element.id).subscribe(
        (response: any) => {
  ...
  private getCompanyById(element: any) {
    ...
    this.subscriptions.push(
      this.companyService.getCompanyById(element.id).subscribe(
        (response: any) => {
